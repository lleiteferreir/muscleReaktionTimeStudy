...
public class PluxDeviceManager : MonoBehaviour
{
    // Class Variables
    private PluxDeviceInterface pluxDevManager;
    ...
    {
        // Initialise object
        pluxDevManager = new PluxDeviceInterface(ScanResults, ConnectionDone, AcquisitionStarted, OnDataReceived, OnEventDetected, OnExceptionRaised);
    ...
    public void StartButtonFunction()
    {
        ...
        List<PluxDeviceInterface.PluxSource> pluxSources = new List<PluxDeviceInterface.PluxSource>();
        ...
        if (pluxDevManager.GetProductIdUnity() == Hybrid8PID)
        {
            ...
            pluxSources.Add(new PluxDeviceInterface.PluxSource(1, 1, resolution, 0x03));
            pluxSources.Add(new PluxDeviceInterface.PluxSource(2, 1, resolution, 0x03));
            ...
            pluxSources.Add(new PluxDeviceInterface.PluxSource(8, 1, resolution, 0x01));

            // Add the sources of the internal IMU channels (CH11 with 9 derivations [3xACC | 3xGYRO | 3xMAG] defined by the 0x01FF chMask).
            int imuPort = 11;
            pluxSources.Add(new PluxDeviceInterface.PluxSource(imuPort, 1, resolution, 0x01FF));
        }
        ...
        else if (pluxDevManager.GetProductIdUnity() == BiosignalspluxPID)
        {
            ...
            pluxSources.Add(new PluxDeviceInterface.PluxSource(1, 1, resolution, 0x01)); // CH1 | EDA
            pluxSources.Add(new PluxDeviceInterface.PluxSource(2, 1, resolution, 0x01)); // CH2 | ECG
        }
        ...
        else if (pluxDevManager.GetProductIdUnity() == MuscleBanPID)
        {
            // Starting a real-time acquisition from:
            // >>> muscleBAN [CH1 > EMG]
            pluxSources.Add(new PluxDeviceInterface.PluxSource(1, 1, resolution, 0x01));
            // >>> muscleBAN [CH2-CH4 > ACC | CH5-CH7 > MAG active]
            pluxSources.Add(new PluxDeviceInterface.PluxSource(2, 1, resolution, 0x3F));
        }
        ...
        else if (pluxDevManager.GetProductIdUnity() == MuscleBanNewPID)
        {
            ...
            pluxSources.Add(new PluxDeviceInterface.PluxSource(11, 1, resolution, 0x3F));
        }
        ...
        else if (pluxDevManager.GetProductIdUnity() == BiosignalspluxSoloPID)
        {
            ...
            pluxSources.Add(new PluxDeviceInterface.PluxSource(2, 1, resolution, 0x01));
    ...
    public void OnEventDetected(PluxDeviceInterface.PluxEvent pluxEvent)
    {
        if (pluxEvent is PluxDeviceInterface.PluxDisconnectEvent)
        {
            // Present an error message.
            OutputMsgText.text =
                "The connection between the computer and the PLUX device was interrupted due to the following event: " +
                (pluxEvent as PluxDeviceInterface.PluxDisconnectEvent).reason;
        ...
        else if (pluxEvent is PluxDeviceInterface.PluxDigInUpdateEvent)
        {
            PluxDeviceInterface.PluxDigInUpdateEvent digInEvent = (pluxEvent as PluxDeviceInterface.PluxDigInUpdateEvent);
